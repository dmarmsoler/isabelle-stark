theory Soundness_FRI_Robust_Query_Phase_Transport
  imports
    Stark.Soundness_FRI_Robust_Actual_Query
...
lemma fri_robust_trace_residual_query_lists_stable_if_no_builder_target:
  assumes ext: "s \<le> t"
    and no_target:
      "\<not> hash_map_new_output_hit
        (fri_checked_builder_merkle_targets data s) s t"
  shows
    "fri_robust_trace_residual_query_lists K data t =
      fri_robust_trace_residual_query_lists K data s"
proof -
  have targets_subset:
...
lemma fri_robust_composition_residual_query_lists_stable_if_no_builder_target:
  assumes ext: "s \<le> t"
    and no_target:
      "\<not> hash_map_new_output_hit
        (fri_checked_builder_merkle_targets data s) s t"
  shows
    "fri_robust_composition_residual_query_lists K data t =
      fri_robust_composition_residual_query_lists K data s"
proof -
  have targets_subset:
...
lemma fri_robust_combined_query_head_lists_stable_if_no_builder_target:
  assumes ext: "s \<le> t"
    and no_target:
      "\<not> hash_map_new_output_hit
        (fri_checked_builder_merkle_targets data s) s t"
  ...
  unfolding fri_robust_combined_query_head_lists_def
    fri_robust_trace_query_head_lists_def
    fri_robust_composition_query_head_lists_def
  using
    fri_robust_trace_residual_query_lists_stable_if_no_builder_target[
      OF ext no_target]
    fri_robust_composition_residual_query_lists_stable_if_no_builder_target[
      OF ext no_target]
  by simp

lemma checked_builder_robust_residual_imp_actual_or_query_phase_target:
...
next
  ...
  have stable:
    ...
    by (rule
      fri_robust_combined_query_head_lists_stable_if_no_builder_target[
        OF _ False])
      (use props in blast)
  have residual_start:
      "map (\<lambda>raw. index (to_nat raw)) raws \<in>
