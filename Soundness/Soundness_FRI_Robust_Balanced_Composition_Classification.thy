theory Soundness_FRI_Robust_Balanced_Composition_Classification
  imports
...
lemma checked_builder_composition_not_robust_close_imp_residual_or_targets_balanced_exact_two:
            ...
            (\<exists>i < length (map fst fl).
              \<not> fri_canonical_layer_close
                ...
                (fri_robust_conditioned_layers (length (map fst fl))
                  (fri_builder_conceptual_layers (map snd fl)
                    attacker_state final))
                (fri_balanced_radius (length (map fst fl)) C) i \<and>
              fri_canonical_layer_close
                (to_nat (staged_degree data))
                (fri_robust_conditioned_layers (length (map fst fl))
                  (fri_builder_conceptual_layers (map snd fl)
                    attacker_state final))
                (fri_balanced_radius (length (map fst fl)) C) (Suc i) \<and>
              ?query_idxs \<in>
