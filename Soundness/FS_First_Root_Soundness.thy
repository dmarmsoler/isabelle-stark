theory FS_First_Root_Soundness
 imports "Stark.FS_First_Root_Alpha" "Stark.FS_Prefix_Soundness"
begin

section \<open>Complete conventional FS soundness with first-root accounting\<close>

text \<open>Replace exactly the outer and nested-alpha first-root target charges.
...
definition fs_first_root_common_error where
  "fs_first_root_common_error Q =
...
definition fs_first_root_nonempty_error where
  "fs_first_root_nonempty_error rT rC Q = fs_first_root_common_error Q +
    nnreal (fs_direct_hash_budget Q)/nnreal size +
    fs_accounted_sampling_error rT rC Q"

lemma fs_compiled_first_root_nonempty_bound:
 ...
 shows "wp_event
   (ro_absorb_checked_staged_security_experiment_with_first_root_and_query_witnesses (fs_compile_replay A P))
     accepted adversary_initial_state \<le> fs_first_root_nonempty_error rT rC Q"
proof -
 ...
 have TraceTarget: "wp_event ?M ?TraceTarget adversary_initial_state\<le>fs_first_root_security_error Q"
   unfolding fs_first_root_security_error_def
   by (rule fs_first_root_security_bound[OF fixed bound nonempty])
 ...
 have Alpha: "wp_event ?M ?Alpha adversary_initial_state\<le>fs_first_root_alpha_error Q"
   by (rule fs_compiled_first_root_alpha_security[OF false_statement fixed bound nonempty])
...
lemma fs_first_root_nonempty_soundness:
  assumes bound: "fs_query_bound Q P" and false_statement: "\<not>exists_valid_trace"
    ...
    and trace_rate: "4*fri_padded_degree_bound (clength-1)\<le>clength*scale"
    and composition_rate: "4*fri_padded_degree_bound maxDegree\<le>clength*scale"
  shows "fs_acceptance_probability P \<le> fs_first_root_nonempty_error rT rC Q"
proof (rule fs_compiled_staged_bound_suffices[OF bound, where A=A])
  ...
  have witnessed: "wp_event
    ...
    accepted adversary_initial_state \<le> fs_first_root_nonempty_error rT rC Q"
    by (rule fs_compiled_first_root_nonempty_bound[OF false_statement fixed capped
...
definition fs_first_root_parameter_error where
  "fs_first_root_parameter_error N rT rC Q =
    (if ro_mca_weighted_parameter_regime N then
      min (fs_prefix_parameter_error N rT rC Q)
        (fs_first_root_nonempty_error rT rC Q)
    else fs_prefix_parameter_error N rT rC Q)"

lemma fs_first_root_parameter_error_le_prefix:
  "fs_first_root_parameter_error N rT rC Q \<le>
    fs_prefix_parameter_error N rT rC Q"
  unfolding fs_first_root_parameter_error_def by simp

lemma fs_first_root_parameter_error_outside_regime:
  "\<not>ro_mca_weighted_parameter_regime N \<Longrightarrow>
    fs_first_root_parameter_error N rT rC Q =
      fs_prefix_parameter_error N rT rC Q"
  unfolding fs_first_root_parameter_error_def by simp

theorem fs_first_root_soundness:
  assumes bound: "fs_query_bound Q P" and false_statement: "\<not>exists_valid_trace"
  shows "fs_acceptance_probability P \<le> fs_first_root_parameter_error N rT rC Q"
proof -
  have current: "fs_acceptance_probability P \<le>
    fs_prefix_parameter_error N rT rC Q"
    by (rule fs_prefix_soundness[OF bound false_statement])
  show ?thesis
  proof (cases "ro_mca_weighted_parameter_regime N")
    ...
    have refined: "fs_acceptance_probability P \<le> fs_first_root_nonempty_error rT rC Q"
      by (rule fs_first_root_nonempty_soundness[OF bound false_statement conditions])
    show ?thesis using current refined True unfolding fs_first_root_parameter_error_def by simp
  next
    case False
    show ?thesis using current False unfolding fs_first_root_parameter_error_def by simp
  qed
qed

lemma fs_first_root_nonempty_error_le_prefix:
 "fs_first_root_nonempty_error rT rC Q \<le> fs_prefix_nonempty_error rT rC Q"
 unfolding fs_first_root_nonempty_error_def fs_first_root_common_error_def
   fs_prefix_nonempty_error_def fs_prefix_common_error_def
 by (intro add_mono order_refl fs_first_root_security_error_le_replay
   fs_first_root_alpha_error_le_prefix)

end
ML \<open>
  val checked = @{thms soundness.fs_compiled_first_root_nonempty_bound
    soundness.fs_first_root_nonempty_soundness soundness.fs_first_root_soundness
    soundness.fs_first_root_nonempty_error_le_prefix
    soundness.fs_first_root_parameter_error_le_prefix
    soundness.fs_first_root_parameter_error_outside_regime};
  if forall (null o Thm.hyps_of) checked andalso null (Thm_Deps.all_oracles checked)
  then () else error "Unexpected first-root soundness proof dependency";
  if Thm.nprems_of @{thm soundness.fs_first_root_soundness} = 3
