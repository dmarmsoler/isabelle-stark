theory Soundness_FRI_Robust_Parameter_Bound
  imports
    Stark.Soundness_FRI_Robust_Security_Classification
    Stark.Soundness_FRI_Conditioned_Explicit_Adaptive_Rectangle_Parameter_Bound
begin

context soundness
begin

definition ro_checked_staged_robust_trace_challenge_error
...
definition ro_checked_staged_robust_composition_challenge_error
  :: "nat \<Rightarrow> staged_budgets \<Rightarrow> prob"
where
...
definition
  ro_absorb_checked_staged_robust_nonempty_parameter_error
    :: "nat \<Rightarrow> staged_budgets \<Rightarrow> prob"
where
...
lemma
  ...
  shows
    ...
    \<le> ro_checked_staged_robust_decoded_semantic_rectangle_error_adaptive
      K budgets"
  by (rule order_trans[
      OF
        wp_ro_absorb_checked_staged_security_query_head_dependent_actual_query_index_list_hit_le
        wp_ro_checked_staged_robust_decoded_semantic_rectangle_bound_adaptive[
          OF wf controlled]])
...
lemma
  ...
  shows
    "wp_event
      ...
      adversary_initial_state
    \<le> ro_checked_staged_robust_combined_actual_residual_rectangle_error_adaptive
      K budgets"
  by (rule order_trans[
      OF
        wp_ro_absorb_checked_staged_security_query_head_dependent_actual_query_index_list_hit_le
        wp_ro_checked_staged_combined_robust_residual_actual_rectangle_bound_adaptive[
          OF wf controlled eval_power trace_exponent_fit
            composition_exponent_fit]])

lemma
  wp_ro_absorb_checked_staged_security_builder_robust_trace_challenge_transition_bound:
  assumes wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
    and eval_power: "clength * scale = 2 ^ N"
    and trace_exponent_fit: "ceil_log clength + K \<le> N"
  ...
  by (rule order_trans[
      OF
        ...
        wp_ro_checked_staged_transcript_with_query_witnesses_robust_conditioned_trace_fri_relation[
          OF wf controlled eval_power trace_exponent_fit trace_capacity
            trace_initial_rate]])

lemma
  wp_ro_absorb_checked_staged_security_builder_robust_composition_challenge_transition_bound:
  assumes wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
    and eval_power: "clength * scale = 2 ^ N"
    and composition_exponent_fit:
      ...
      OF
        ...
        wp_ro_checked_staged_transcript_with_query_witnesses_robust_conditioned_composition_fri_relation[
          OF wf controlled eval_power composition_exponent_fit
            composition_capacity composition_initial_rate]])
...
lemma
  ...
  by (rule order_trans[
      OF
        ...
        wp_ro_checked_staged_first_root_robust_decoded_all_queries_consistent_bound[
          OF false_statement wf controlled nonempty]])



lemma
  wp_ro_absorb_checked_staged_security_acceptance_robust_nonempty_parameter_bound:
  assumes false_statement: "\<not> exists_valid_trace"
    and wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
    and nonempty: "0 < ceil_log clength"
    ...
    by (rule
      wp_ro_absorb_checked_staged_security_acceptance_le_robust_obstruction_union[
        OF wf controlled nonempty eval_power trace_exponent_fit
          ...
          trace_initial_rate composition_initial_rate])
...
lemma ro_absorb_stark_soundness_robust_nonempty:
  assumes false_statement: "\<not> exists_valid_trace"
    and wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
    and nonempty: "0 < ceil_log clength"
    ...
    by (rule
      wp_ro_absorb_checked_staged_security_acceptance_robust_nonempty_parameter_bound[
        OF false_statement wf controlled nonempty eval_power
          trace_exponent_fit composition_exponent_fit trace_capacity
          composition_capacity trace_initial_rate composition_initial_rate])
  have projection:
    "wp_event
        (ro_absorb_checked_staged_security_experiment_with_first_root_and_query_witnesses
          A)
...
definition
  ro_absorb_checked_staged_robust_tightened_nonempty_parameter_error
    :: "nat \<Rightarrow> staged_budgets \<Rightarrow> prob"
where
  "ro_absorb_checked_staged_robust_tightened_nonempty_parameter_error K budgets =
    min
...
lemma
  ro_absorb_checked_staged_robust_tightened_nonempty_parameter_error_le_previous:
  "ro_absorb_checked_staged_robust_tightened_nonempty_parameter_error K budgets
    \<le>
    ro_absorb_checked_staged_conditioned_explicit_complete_list_adaptive_rectangle_tightened_nonempty_parameter_error
      budgets"
  unfolding
    ro_absorb_checked_staged_robust_tightened_nonempty_parameter_error_def
  by simp

lemma
  wp_ro_absorb_checked_staged_security_acceptance_robust_tightened_nonempty_parameter_bound:
  assumes false_statement: "\<not> exists_valid_trace"
    and wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
    and nonempty: "0 < ceil_log clength"
...
lemma ro_absorb_stark_soundness_robust_tightened_nonempty:
  assumes false_statement: "\<not> exists_valid_trace"
    and wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
    and nonempty: "0 < ceil_log clength"
