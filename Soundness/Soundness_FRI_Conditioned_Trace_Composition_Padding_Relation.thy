theory Soundness_FRI_Conditioned_Trace_Composition_Padding_Relation
  imports
...
lemma trace_composition_query_index_bound_for_mono:
  assumes degree: "d \<le> D"
  shows
  ...
  by (rule div_le_mono)
    (simp add: query_agreement_bound_for_mono[OF degree])

definition trace_composition_padding_query_index_bound :: nat
where
  "trace_composition_padding_query_index_bound =
    trace_composition_query_index_bound_for
      (fri_padded_degree_bound maxDegree)"

definition trace_composition_padding_query_raw_fiber_bound :: nat
where
  "trace_composition_padding_query_raw_fiber_bound =
    query_raw_preimage_card_envelope
      trace_composition_padding_query_index_bound"

definition ro_trace_composition_padding_header_query_lists
where
  "ro_trace_composition_padding_header_query_lists M fr trace_roots trace_final
...
lemma ro_trace_composition_padding_header_query_lists_query_index_update[simp]:
  "ro_trace_composition_padding_header_query_lists
    ...
    ro_trace_composition_padding_header_query_lists M
      fr trace_roots trace_final as dg composition_roots composition_final"
...
lemma ro_trace_composition_padding_header_query_lists_cong:
  assumes
...
lemma ro_trace_composition_padding_header_query_lists_fresh_update:
  ...
  shows
    "ro_trace_composition_padding_header_query_lists (fmupd x y M)
        fr trace_roots trace_final as dg composition_roots composition_final =
      ro_trace_composition_padding_header_query_lists M
        fr trace_roots trace_final as dg composition_roots composition_final"
proof -
...
lemma trace_composition_padding_query_index_bound_mono:
...
proof -
  ...
  show ?thesis
    ...
    by (rule trace_composition_query_index_bound_for_mono[OF padded])
qed

lemma ro_trace_composition_padding_header_query_lists_position_card_bound:
    ...
    "card
      (query_index_raw_list_position_values
        (ro_trace_composition_padding_header_query_lists M
          ...
          composition_final)
        i)
      \<le> trace_composition_padding_query_raw_fiber_bound"
proof (cases "to_nat dg \<le> maxDegree")
  ...
  proof (cases
    ...
    then have trace_low: "trace_table_low_degree ?candidate"
      and composition_low:
        "composition_table_low_degree ?D ?composition"
    ...
    have raw_card:
      ...
      by (rule card_query_index_raw_preimage_le_query_envelope[
        OF indices_subset])
    have indices_card:
        "card ?indices \<le> trace_composition_query_index_bound_for ?D"
      by (rule trace_composition_accepted_indices_card_bound_for[
        OF lengths trace_low composition_low composition_not_low])
    have uniform:
        "trace_composition_query_index_bound_for ?D \<le>
          trace_composition_padding_query_index_bound"
    ...
    have indices_uniform:
        "card ?indices \<le> trace_composition_padding_query_index_bound"
    ...
    show ?thesis
      unfolding trace_composition_padding_query_raw_fiber_bound_def
      by (rule order_trans[OF card_le])
        (rule order_trans[OF raw_card envelope])
  next
    assume not_condition:
        "\<not> (trace_table_low_degree ?candidate \<and>
          composition_table_low_degree ?D ?composition \<and>
          \<not> composition_table_low_degree maxDegree ?composition)"
    have Q_empty: "?Q = {}"
      unfolding ro_trace_composition_padding_header_query_lists_def
...
definition trace_composition_padding_absorbed_query_relation
  :: "(('f protocol_hash_input, 'f) fmap \<Rightarrow>
      'f protocol_hash_input \<Rightarrow> 'f \<Rightarrow> bool)"
where
  "trace_composition_padding_absorbed_query_relation M x y \<longleftrightarrow>
...
lemma trace_composition_padding_absorbed_query_relationD:
  assumes rel: "trace_composition_padding_absorbed_query_relation M x y"
  obtains fr trace_roots trace_final as dg composition_roots
    ...
    "y = raws ! j"
  using rel
  unfolding trace_composition_padding_absorbed_query_relation_def Let_def
  by blast

lemma trace_composition_padding_absorbed_query_relation_direct_fiber_card_bound:
  "card {y.
      hash_state_relation_direct_activation
        trace_composition_padding_absorbed_query_relation M x y}
    \<le> trace_composition_padding_query_raw_fiber_bound"
proof (cases "{y.
    hash_state_relation_direct_activation
      trace_composition_padding_absorbed_query_relation M x y} = {}")
...
next
  ...
  then obtain y0 where y0:
    "hash_state_relation_direct_activation
      trace_composition_padding_absorbed_query_relation M x y0"
    by blast
  have rel0:
      "trace_composition_padding_absorbed_query_relation (fmupd x y0 M) x y0"
    using y0
    unfolding hash_state_relation_direct_activation_def
      hash_state_relation_active_def
    by blast
  from trace_composition_padding_absorbed_query_relationD[OF rel0]
  obtain fr0 trace_roots0 trace_final0 as0 dg0 composition_roots0
    ...
    and raws_in0:
      "map (\<lambda>raw. index (to_nat raw)) raws0 \<in>
        ro_trace_composition_padding_header_query_lists (fmupd x y0 M)
    ...
    assume y:
      "y \<in> {y.
        hash_state_relation_direct_activation
          trace_composition_padding_absorbed_query_relation M x y}"
    have rely:
        "trace_composition_padding_absorbed_query_relation (fmupd x y M) x y"
      using y
      unfolding hash_state_relation_direct_activation_def
        hash_state_relation_active_def
      by blast
    from trace_composition_padding_absorbed_query_relationD[OF rely]
  ...
  have position_le:
    ...
    unfolding x0
    by (rule
        ro_trace_composition_padding_header_query_lists_position_card_bound[
          OF j0_bound])
  show ?thesis
    by (rule order_trans[OF card_le position_le])
qed

lemma trace_composition_padding_absorbed_query_relation_fresh_update_pullback:
  assumes fresh: "fmlookup M x = None"
    and key_neq: "k \<noteq> x"
    and active_lookup: "fmlookup (fmupd x y M) k = Some z"
    and rel:
      "trace_composition_padding_absorbed_query_relation (fmupd x y M) k z"
    and no_target: "y \<notin> first_root_relation_drift_targets M x"
  shows "trace_composition_padding_absorbed_query_relation M k z"
proof -
  from trace_composition_padding_absorbed_query_relationD[OF rel]
  obtain fr trace_roots trace_final as dg composition_roots
      composition_final query_chunks raws query_start final j where
    clean_new:
      "\<not> hash_map_output_collision
  ...
  have query_lists_eq:
      "ro_trace_composition_padding_header_query_lists (fmupd x y M)
        ...
        ro_trace_composition_padding_header_query_lists M
          fr trace_roots trace_final as dg composition_roots
          composition_final"
    by (rule ro_trace_composition_padding_header_query_lists_fresh_update[
...
lemma trace_composition_padding_absorbed_query_relation_drift_activation_imp_target:
  assumes fresh: "fmlookup M x = None"
    and drift:
...
proof (rule ccontr)
  ...
  from drift obtain k z where
    ...
    and inactive_old:
      "\<not> hash_state_relation_active
        trace_composition_padding_absorbed_query_relation M k z"
  ...
  have rel_old:
      "trace_composition_padding_absorbed_query_relation M k z"
    by (rule
      trace_composition_padding_absorbed_query_relation_fresh_update_pullback[
...
lemma trace_composition_padding_absorbed_query_relation_drift_fiber_card_bound:
  assumes fresh: "fmlookup M x = None"
  shows
    "card {y.
      hash_state_relation_drift_activation
...
proof -
  have subset:
    ...
    using
      trace_composition_padding_absorbed_query_relation_drift_activation_imp_target[
        OF fresh]
    by blast
  have
      "card {y.
...
lemma trace_composition_padding_bounded_transition_fiber_card_bound:
  assumes fresh: "fmlookup M x = None"
  shows
    "card {y.
      hash_state_relation_transition
        ...
        M (fmupd x y M)}
      \<le> trace_composition_padding_query_raw_fiber_bound + (5 * L + 2)"
proof (rule conditioned_fri_bounded_transition_fiber_card_bound[
  ...
  show
    "card {y.
      ...
      \<le> trace_composition_padding_query_raw_fiber_bound"
    by (rule
      trace_composition_padding_absorbed_query_relation_direct_fiber_card_bound)
next
  ...
  have
    "card {y.
      ...
      \<le> 5 * card (fmdom' M) + 2"
    by (rule
      trace_composition_padding_absorbed_query_relation_drift_fiber_card_bound[
        OF fresh])
  also have "... \<le> 5 * L + 2"
...
lemma
  ...
  shows
    "wp_event
      ...
      \<le> nnreal
        (q * (trace_composition_padding_query_raw_fiber_bound + (5 * q + 2))) /
        nnreal size"
proof -
  let ?Q = "ro_checked_staged_transcript_hash_query_budget_for budgets"
  let ?B =
    "trace_composition_padding_query_raw_fiber_bound + (5 * ?Q + 2)"
  have steps:
      "\<And>M x. fmlookup M x = None \<Longrightarrow>
        card {y.
          hash_state_relation_transition
            ...
            M (fmupd x y M)}
          \<le> ?B"
    by (rule trace_composition_padding_bounded_transition_fiber_card_bound)
  have bound:
      "wp_event
        (ro_checked_staged_transcript_program_with_first_root_and_query_witnesses
          A)
...
lemma trace_composition_padding_absorbed_query_relation_activeI:
  assumes clean:
      "\<not> hash_map_output_collision (channel_for_hash_map M)"
    and no_initial:
      "PState adversary_initial_state \<notin>
  ...
  shows
    "hash_state_relation_active
      trace_composition_padding_absorbed_query_relation M
