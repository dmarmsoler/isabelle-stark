theory Soundness_FRI_Robust_Balanced_Strict_Selected_Actual_Query
  imports Stark.Soundness_FRI_Robust_Balanced_Strict_Selected_Residual
begin

context soundness
begin

lemma fri_balanced_strict_selected_residual_index_card_bound_mono:
  assumes "d \<le> D"
  shows
    "fri_balanced_strict_selected_residual_index_card_bound d C \<le>
...
definition fri_balanced_strict_selected_trace_residual_query_lists
  :: "nat \<Rightarrow> 'f staged_proof_data \<Rightarrow>
    'f protocol_channel \<Rightarrow> nat list set"
where
  "fri_balanced_strict_selected_trace_residual_query_lists C data query_start =
...
definition fri_balanced_strict_selected_composition_residual_query_lists
  :: "nat \<Rightarrow> 'f staged_proof_data \<Rightarrow>
    'f protocol_channel \<Rightarrow> nat list set"
where
  "fri_balanced_strict_selected_composition_residual_query_lists
      C data query_start =
...
definition fri_balanced_strict_selected_trace_query_head_lists
  :: "nat \<Rightarrow> ('f \<times> 'f list \<times> 'f) \<Rightarrow>
    'f protocol_channel \<Rightarrow> 'f staged_proof_data \<Rightarrow>
    'f protocol_channel \<Rightarrow> nat list set"
where
  "fri_balanced_strict_selected_trace_query_head_lists C
      prefix prefix_state data query_start =
    fri_balanced_strict_selected_trace_residual_query_lists C data query_start"

definition fri_balanced_strict_selected_composition_query_head_lists
  :: "nat \<Rightarrow> ('f \<times> 'f list \<times> 'f) \<Rightarrow>
    'f protocol_channel \<Rightarrow> 'f staged_proof_data \<Rightarrow>
    'f protocol_channel \<Rightarrow> nat list set"
where
...
definition fri_balanced_strict_selected_combined_query_head_lists
  :: "nat \<Rightarrow> ('f \<times> 'f list \<times> 'f) \<Rightarrow>
    'f protocol_channel \<Rightarrow> 'f staged_proof_data \<Rightarrow>
    'f protocol_channel \<Rightarrow> nat list set"
where
...
definition ro_checked_staged_balanced_strict_selected_residual_rectangle_error_adaptive
  :: "nat \<Rightarrow> nat \<Rightarrow> staged_budgets \<Rightarrow> prob"
where
  "ro_checked_staged_balanced_strict_selected_residual_rectangle_error_adaptive
      d C budgets =
    (nnreal
...
lemma wp_ro_checked_staged_trace_balanced_strict_selected_residual_actual_rectangle_bound_adaptive:
  ...
  shows
    ...
    \<le> ro_checked_staged_balanced_strict_selected_residual_rectangle_error_adaptive
        (clength - 1) C budgets"
  unfolding
    ...
    wp_ro_query_head_dependent_actual_query_index_list_hit_adaptive_rectangle_bound[
      ...
      where
        I="\<lambda>prefix prefix_state data query_start.
          fri_balanced_strict_selected_residual_query_indices (clength - 1) C
            (staged_trace_fri_roots data)
            (staged_trace_fri_challenges data)
            (staged_trace_final data) query_start"])
    ...
    "fri_balanced_strict_selected_trace_query_head_lists C
      ...
      \<subseteq> fri_conditioned_query_lists
        (fri_balanced_strict_selected_residual_query_indices (clength - 1) C
          ...
          (staged_trace_fri_challenges data)
          (staged_trace_final data) query_start)"
    unfolding fri_balanced_strict_selected_trace_query_head_lists_def
      fri_balanced_strict_selected_trace_residual_query_lists_def
    by (rule
      fri_balanced_strict_selected_residual_query_lists_rectangle_cover[
...
next
  ...
  show
    ...
    by (rule fri_balanced_strict_selected_residual_query_indices_subset)
next
  fix prefix prefix_state data query_start t
  assume head:
    "Some (((prefix, prefix_state), data, query_start), t) \<in>
  ...
  show
    "card
      (fri_balanced_strict_selected_residual_query_indices (clength - 1) C
        ...
        (staged_trace_fri_challenges data)
        (staged_trace_final data) query_start)
      \<le> fri_balanced_strict_selected_residual_index_card_bound
          (clength - 1) C"
    by (rule card_fri_balanced_strict_selected_residual_query_indices[
          OF eval_power round_count rounds_fit lengths])
qed


lemma wp_ro_checked_staged_composition_balanced_strict_selected_residual_actual_rectangle_bound_adaptive:
  assumes wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
    and eval_power: "clength * scale = 2 ^ N"
...
next
  ...
  show
    ...
    by (cases "to_nat (staged_degree data) \<le> maxDegree")
      (simp_all add:
        fri_balanced_strict_selected_residual_query_indices_subset)
next
  fix prefix prefix_state data query_start t
  assume head:
    "Some (((prefix, prefix_state), data, query_start), t) \<in>
  ...
  show
    "card
      ...
      \<le> fri_balanced_strict_selected_residual_index_card_bound maxDegree C"
  proof (cases "to_nat (staged_degree data) \<le> maxDegree")
    case True
    have shape:
        "length (staged_composition_fri_roots data) =
    ...
    have local:
      ...
      by (rule card_fri_balanced_strict_selected_residual_query_indices[
            OF eval_power round_count rounds_fit lengths])
    have mono:
        "fri_balanced_strict_selected_residual_index_card_bound
            (to_nat (staged_degree data)) C
          \<le> fri_balanced_strict_selected_residual_index_card_bound
              maxDegree C"
      by (rule
        fri_balanced_strict_selected_residual_index_card_bound_mono[OF True])
    show ?thesis
...
lemma wp_ro_checked_staged_combined_balanced_strict_selected_residual_actual_rectangle_bound_adaptive:
  assumes wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
    and eval_power: "clength * scale = 2 ^ N"
    and trace_rounds_fit: "Suc (ceil_log clength) \<le> N"
...
proof -
  ...
  have event_eq:
    ...
    unfolding ro_query_head_dependent_actual_query_index_list_hit_def
      fri_balanced_strict_selected_combined_query_head_lists_def
  ...
  have trace:
      "wp_event ?M ?Trace adversary_initial_state \<le>
        ro_checked_staged_balanced_strict_selected_residual_rectangle_error_adaptive
          (clength - 1) C budgets"
    by (rule
      wp_ro_checked_staged_trace_balanced_strict_selected_residual_actual_rectangle_bound_adaptive[
  ...
  have composition:
      "wp_event ?M ?Composition adversary_initial_state \<le>
        ro_checked_staged_balanced_strict_selected_residual_rectangle_error_adaptive
          maxDegree C budgets"
    by (rule
      wp_ro_checked_staged_composition_balanced_strict_selected_residual_actual_rectangle_bound_adaptive[
