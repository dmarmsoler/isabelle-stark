theory Soundness_FRI_Robust_Balanced_Actual_Query
  imports
...
lemma finite_fri_balanced_residual_active_layers[simp]:
  "finite (fri_balanced_residual_active_layers d C roots challenges
    final_value prefix_state)"
  unfolding fri_balanced_residual_active_layers_def
  by (rule finite_subset[OF _ finite_lessThan[of "length challenges"]])
    auto

definition fri_balanced_quantitative_residual_query_lists
  :: "nat \<Rightarrow> nat \<Rightarrow> 'f list \<Rightarrow> 'f list \<Rightarrow>
    'f \<Rightarrow> 'f protocol_channel \<Rightarrow> nat list set"
where
  "fri_balanced_quantitative_residual_query_lists d C roots challenges
      final_value prefix_state =
    {qs \<in> fri_query_index_list_space.
      \<exists>i \<in> fri_balanced_residual_active_layers
          d C roots challenges final_value prefix_state.
        qs \<in> fri_robust_conditioned_residual_query_lists
          roots challenges
          (fri_builder_conceptual_layers roots prefix_state final_value) i}"

lemma fri_balanced_quantitative_residual_query_lists_subset:
  "fri_balanced_quantitative_residual_query_lists d C roots challenges
      final_value prefix_state \<subseteq> fri_query_index_list_space"
  unfolding fri_balanced_quantitative_residual_query_lists_def by auto

lemma fri_balanced_quantitative_residual_query_lists_rectangle_cover:
  ...
  shows
    "fri_balanced_quantitative_residual_query_lists d C roots challenges
        final_value prefix_state
      \<subseteq>
      (\<Union>i\<in>fri_balanced_residual_active_layers d C roots challenges
          final_value prefix_state.
...
proof
  ...
  fix qs
  assume qs_in:
      "qs \<in> fri_balanced_quantitative_residual_query_lists
        d C roots challenges final_value prefix_state"
  then obtain i where active:
    ...
    unfolding fri_balanced_quantitative_residual_query_lists_def by blast
  ...
  show
      "qs \<in>
        (\<Union>i\<in>fri_balanced_residual_active_layers
            d C roots challenges final_value prefix_state.
          fri_conditioned_query_lists
            (fri_balanced_residual_layer_query_indices roots challenges
              ?layers i))"
...
lemma card_fri_balanced_residual_layer_query_indices_active:
  assumes eval_power: "clength * scale = 2 ^ N"
...
definition fri_balanced_residual_rectangle_layer_raw_card_bound
  :: "nat \<Rightarrow> nat \<Rightarrow> nat"
where
  "fri_balanced_residual_rectangle_layer_raw_card_bound C i =
...
definition ro_checked_staged_balanced_residual_rectangle_error_adaptive
  :: "nat \<Rightarrow> nat \<Rightarrow> staged_budgets \<Rightarrow> prob"
where
  "ro_checked_staged_balanced_residual_rectangle_error_adaptive d C budgets =
...
lemma fri_balanced_residual_active_layer_probability_sum_le:
  assumes eval_power: "clength * scale = 2 ^ N"
    and round_count: "length challenges = ceil_log (Suc d)"
    and rounds_fit: "Suc (length challenges) \<le> N"
    and lengths: "length challenges = length roots"
  shows
    "(\<Sum>i\<in>fri_balanced_residual_active_layers
        d C roots challenges final_value prefix_state.
      (nnreal
          (query_raw_preimage_card_envelope
            (card
        ...
        nnreal size) ^
      (rounds - staged_attacker_query_budget budgets))
    \<le> ro_checked_staged_balanced_residual_rectangle_error_adaptive
        d C budgets"
proof -
  let ?A =
  ...
  let ?g =
    "\<lambda>i.
      (nnreal (fri_balanced_residual_rectangle_layer_raw_card_bound C i) /
        nnreal size) ^
      (rounds - staged_attacker_query_budget budgets)"
  ...
  proof (rule sum_mono)
    ...
    show "?f i \<le> ?g i"
      unfolding
        fri_balanced_residual_rectangle_layer_raw_card_bound_def Let_def
  ...
  show ?thesis
    unfolding
      ro_checked_staged_balanced_residual_rectangle_error_adaptive_def Let_def
    by (rule order_trans[OF local extend])
qed


definition fri_balanced_trace_residual_query_lists
  :: "nat \<Rightarrow> 'f staged_proof_data \<Rightarrow>
    'f protocol_channel \<Rightarrow> nat list set"
where
  "fri_balanced_trace_residual_query_lists C data query_start =
...
definition fri_balanced_composition_residual_query_lists
  :: "nat \<Rightarrow> 'f staged_proof_data \<Rightarrow>
    'f protocol_channel \<Rightarrow> nat list set"
where
  "fri_balanced_composition_residual_query_lists C data query_start =
...
lemma fri_balanced_trace_residual_query_lists_subset:
  "fri_balanced_trace_residual_query_lists C data query_start
    \<subseteq> fri_query_index_list_space"
  unfolding fri_balanced_trace_residual_query_lists_def
  by (rule fri_balanced_quantitative_residual_query_lists_subset)

lemma fri_balanced_composition_residual_query_lists_subset:
  "fri_balanced_composition_residual_query_lists C data query_start
    \<subseteq> fri_query_index_list_space"
...
definition fri_balanced_trace_query_head_lists
...
definition fri_balanced_composition_query_head_lists
  :: "nat \<Rightarrow> ('f \<times> 'f list \<times> 'f) \<Rightarrow>
    'f protocol_channel \<Rightarrow> 'f staged_proof_data \<Rightarrow>
    'f protocol_channel \<Rightarrow> nat list set"
where
  "fri_balanced_composition_query_head_lists C
      prefix prefix_state data query_start =
    fri_balanced_composition_residual_query_lists C data query_start"

definition fri_balanced_combined_query_head_lists
  :: "nat \<Rightarrow> ('f \<times> 'f list \<times> 'f) \<Rightarrow>
    'f protocol_channel \<Rightarrow> 'f staged_proof_data \<Rightarrow>
    'f protocol_channel \<Rightarrow> nat list set"
where
...
lemma ro_checked_staged_balanced_residual_rectangle_error_adaptive_mono:
  assumes "d \<le> D"
  shows
    "ro_checked_staged_balanced_residual_rectangle_error_adaptive
        d C budgets
      \<le> ro_checked_staged_balanced_residual_rectangle_error_adaptive
        D C budgets"
  unfolding
    ro_checked_staged_balanced_residual_rectangle_error_adaptive_def
  by (rule sum_mono2)
    ...
    wp_ro_query_head_dependent_actual_query_index_list_hit_adaptive_rectangle_family_bound[
      ...
      where
        K="\<lambda>prefix prefix_state data query_start.
          fri_balanced_residual_active_layers (clength - 1) C
            (staged_trace_fri_roots data)
            (staged_trace_fri_challenges data)
            (staged_trace_final data) query_start"
      and
...
next
  ...
  show
    "fri_balanced_trace_query_head_lists C
        prefix prefix_state data query_start
      \<subseteq>
      (\<Union>i\<in>fri_balanced_residual_active_layers (clength - 1) C
          (staged_trace_fri_roots data)
          (staged_trace_fri_challenges data)
          (staged_trace_final data) query_start.
        fri_conditioned_query_lists
    ...
    by (rule fri_balanced_quantitative_residual_query_lists_rectangle_cover[
...
next
  ...
  have lengths:
    ...
    using shape by simp
  show
    "(\<Sum>i\<in>fri_balanced_residual_active_layers (clength - 1) C
        (staged_trace_fri_roots data)
        (staged_trace_fri_challenges data)
        (staged_trace_final data) query_start.
      (nnreal
    ...
    \<le> ro_checked_staged_balanced_residual_rectangle_error_adaptive
        (clength - 1) C budgets"
    by (rule fri_balanced_residual_active_layer_probability_sum_le[
...
next
  ...
  proof (cases "to_nat (staged_degree data) \<le> maxDegree")
    ...
    have lengths:
      ...
      using shape by simp
    show ?thesis
      unfolding fri_balanced_composition_query_head_lists_def
        fri_balanced_composition_residual_query_lists_def
        if_P[OF True]
      by (rule
          fri_balanced_quantitative_residual_query_lists_rectangle_cover[
            OF eval_power roots_le lengths])
  next
    case False
    show ?thesis
  ...
  proof (cases "to_nat (staged_degree data) \<le> maxDegree")
    ...
    have local:
      "(\<Sum>i\<in>
        ...
        (rounds - staged_attacker_query_budget budgets))
      \<le> ro_checked_staged_balanced_residual_rectangle_error_adaptive
          (to_nat (staged_degree data)) C budgets"
      by (rule fri_balanced_residual_active_layer_probability_sum_le[
            OF eval_power round_count rounds_fit lengths])
    have mono:
        "ro_checked_staged_balanced_residual_rectangle_error_adaptive
          ...
          \<le> ro_checked_staged_balanced_residual_rectangle_error_adaptive
              maxDegree C budgets"
      by (rule
          ro_checked_staged_balanced_residual_rectangle_error_adaptive_mono[
            OF True])
    show ?thesis
...
definition
  ro_checked_staged_balanced_combined_actual_residual_rectangle_error_adaptive
...
proof -
  ...
  have event_eq:
    ...
    unfolding ro_query_head_dependent_actual_query_index_list_hit_def
      fri_balanced_combined_query_head_lists_def
  ...
  show ?thesis
    unfolding
      ro_checked_staged_balanced_combined_actual_residual_rectangle_error_adaptive_def
