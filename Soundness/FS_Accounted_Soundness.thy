(* Title: Stark/FS_Accounted_Soundness.thy
   License: BSD-3-Clause *)

theory FS_Accounted_Soundness
  imports "FS_Replay_Soundness" "FS_Compiled_Event_Accounting"
    "FS_Residual_Accounting"
begin

section \<open>Complete conventional Fiat--Shamir soundness with replay-aware accounting\<close>

text \<open>Only the final collision, initial fixed-target and weighted clean-residual
  charges use the direct producer-plus-verifier budget. The seven remaining
  classification branches retain their existing staged allowances. No prefix
  stability or independence is inferred merely from equality of final maps.
  The conventional producer may adapt to all prior answers and use private
  randomness, in the existing terminating finite-support classical model.

  The final minimum retains the previous public bound and its regime fallback.
  It is never larger, but no unconditional strict improvement or new concrete
  security profile is asserted. The compiler and its operational allowances
  are unchanged.\<close>

context soundness
begin

definition fs_accounted_sampling_error where
...
definition fs_accounted_common_error where
...
definition fs_accounted_nonempty_error where
  "fs_accounted_nonempty_error rT rC Q = fs_accounted_common_error Q +
    nnreal (fs_direct_hash_budget Q)/nnreal size +
    fs_accounted_sampling_error rT rC Q"

lemma fs_compiled_accounted_nonempty_bound:
 ...
 shows "wp_event
   (ro_absorb_checked_staged_security_experiment_with_first_root_and_query_witnesses (fs_compile_replay A P))
     accepted adversary_initial_state \<le> fs_accounted_nonempty_error rT rC Q"
proof -
 ...
 have TraceChallenge: "wp_event ?M ?TraceChallenge adversary_initial_state\<le>ro_mca_challenge_error (fs_replay_budgets Q)"
   by (rule wp_ro_mca_trace_security_bad[OF wf controlled eval_power trace_rounds_fit trace_rate])
 have CompositionChallenge: "wp_event ?M ?CompositionChallenge adversary_initial_state\<le>ro_mca_challenge_error (fs_replay_budgets Q)"
 ...
 have Alpha: "wp_event ?M ?Alpha adversary_initial_state\<le>ro_checked_staged_first_root_robust_alpha_pivot_error (fs_replay_budgets Q)"
 ...
 have closed: "sum_list(map (\<lambda>E. wp_event ?M E adversary_initial_state) ?Es)\<le>
     fs_accounted_nonempty_error rT rC Q"
   unfolding fs_accounted_nonempty_error_def fs_accounted_common_error_def
...
lemma fs_accounted_nonempty_soundness:
  assumes bound: "fs_query_bound Q P" and false_statement: "\<not>exists_valid_trace"
    ...
    and trace_rate: "4*fri_padded_degree_bound (clength-1)\<le>clength*scale"
    and composition_rate: "4*fri_padded_degree_bound maxDegree\<le>clength*scale"
  shows "fs_acceptance_probability P \<le> fs_accounted_nonempty_error rT rC Q"
proof (rule fs_compiled_staged_bound_suffices[OF bound, where A=A])
  ...
  have witnessed: "wp_event
    ...
    accepted adversary_initial_state \<le> fs_accounted_nonempty_error rT rC Q"
    by (rule fs_compiled_accounted_nonempty_bound[OF false_statement fixed capped
...
definition fs_accounted_parameter_error where
  "fs_accounted_parameter_error N rT rC Q =
    (if ro_mca_weighted_parameter_regime N then
      min (ro_mca_weighted_parameter_error N rT rC (fs_replay_budgets Q))
        (fs_accounted_nonempty_error rT rC Q)
    else ro_mca_weighted_parameter_error N rT rC (fs_replay_budgets Q))"

lemma fs_accounted_parameter_error_le_replay:
  "fs_accounted_parameter_error N rT rC Q \<le>
    ro_mca_weighted_parameter_error N rT rC (fs_replay_budgets Q)"
  unfolding fs_accounted_parameter_error_def by simp

lemma fs_accounted_parameter_error_outside_regime:
  "\<not>ro_mca_weighted_parameter_regime N \<Longrightarrow>
    fs_accounted_parameter_error N rT rC Q =
      ro_mca_weighted_parameter_error N rT rC (fs_replay_budgets Q)"
  unfolding fs_accounted_parameter_error_def by simp

theorem fs_accounted_soundness:
  assumes bound: "fs_query_bound Q P" and false_statement: "\<not>exists_valid_trace"
  shows "fs_acceptance_probability P \<le> fs_accounted_parameter_error N rT rC Q"
proof -
  have current: "fs_acceptance_probability P \<le>
    ro_mca_weighted_parameter_error N rT rC (fs_replay_budgets Q)"
    by (rule fs_replay_soundness[OF bound false_statement])
  show ?thesis
  proof (cases "ro_mca_weighted_parameter_regime N")
    ...
    have refined: "fs_acceptance_probability P \<le> fs_accounted_nonempty_error rT rC Q"
      by (rule fs_accounted_nonempty_soundness[OF bound false_statement conditions])
    show ?thesis using current refined True unfolding fs_accounted_parameter_error_def by simp
  next
    case False
    show ?thesis using current False unfolding fs_accounted_parameter_error_def by simp
  qed
qed
