theory Staged_Security_Experiment_RO_Transcript_Collision_Budgets
  imports Staged_Security_Experiment_RO_Transcript_Budgets
begin

context soundness
begin

definition hash_range_collision_budget ::
  "nat \<Rightarrow> ('x, 'f protocol_channel) state_monad \<Rightarrow> bool" where
  "hash_range_collision_budget q m \<longleftrightarrow>
    hash_range_budget q m \<and> hash_collision_budget q m"

lemma hash_range_collision_budgetI:
  assumes range: "hash_range_budget q m"
    and collision: "hash_collision_budget q m"
  shows "hash_range_collision_budget q m"
  using range collision
  unfolding hash_range_collision_budget_def by simp

lemma hash_range_collision_budget_range:
  assumes "hash_range_collision_budget q m"
  shows "hash_range_budget q m"
  using assms unfolding hash_range_collision_budget_def by simp

lemma hash_range_collision_budget_collision:
  assumes "hash_range_collision_budget q m"
  shows "hash_collision_budget q m"
  using assms unfolding hash_range_collision_budget_def by simp

lemma hash_range_collision_budget_return:
  "hash_range_collision_budget 0 (return x)"
  by (rule hash_range_collision_budgetI)
    (rule hash_range_budget_return, rule hash_collision_budget_return)

lemma hash_range_collision_budget_bind:
  assumes m: "hash_range_collision_budget n m"
    and k: "\<And>x. hash_range_collision_budget n' (k x)"
  shows "hash_range_collision_budget (n + n') (m \<bind> k)"
proof (rule hash_range_collision_budgetI)
  show "hash_range_budget (n + n') (m \<bind> k)"
    by (rule hash_range_budget_bind)
      (rule hash_range_collision_budget_range[OF m],
        rule hash_range_collision_budget_range[OF k])
  show "hash_collision_budget (n + n') (m \<bind> k)"
    by (rule hash_collision_budget_bind)
      (rule hash_range_collision_budget_range[OF m],
        rule hash_range_collision_budget_collision[OF m],
        rule hash_range_collision_budget_range[OF k],
        rule hash_range_collision_budget_collision[OF k])
qed

lemma hash_range_collision_budget_bind_on_outcomes:
  assumes m: "hash_range_collision_budget n m"
    and k:
      "\<And>s x t. Some (x, t) \<in> set_dist (execute m s) \<Longrightarrow>
        hash_range_collision_budget n' (k x)"
  shows "hash_range_collision_budget (n + n') (m \<bind> k)"
proof (rule hash_range_collision_budgetI)
  show "hash_range_budget (n + n') (m \<bind> k)"
  ...
  next
    ...
    show "hash_range_budget n' (k x)"
      by (rule hash_range_collision_budget_range[OF k[OF out]])
  qed
  show "hash_collision_budget (n + n') (m \<bind> k)"
  ...
  next
    ...
    show "hash_range_budget n' (k x)"
      by (rule hash_range_collision_budget_range[OF k[OF out]])
    show "hash_collision_budget n' (k x)"
      by (rule hash_range_collision_budget_collision[OF k[OF out]])
  qed
qed

lemma controlled_ro_program_range_collision:
  assumes "controlled_ro_program q m"
  shows "hash_range_collision_budget q m"
  by (rule hash_range_collision_budgetI)
    (rule controlled_ro_program_range[OF assms],
      rule controlled_ro_program_collision[OF assms])

lemma hash_range_collision_budget_ro_record_staged_message:
  "hash_range_collision_budget 1 (ro_record_staged_message msg)"
  by (rule hash_range_collision_budgetI)
    (rule hash_range_budget_ro_record_staged_message,
      rule hash_collision_budget_ro_record_staged_message)

lemma hash_range_collision_budget_ro_staged_alpha_program:
  "hash_range_collision_budget (2 * n) (ro_staged_alpha_program n)"
  by (rule hash_range_collision_budgetI)
    (rule hash_range_budget_ro_staged_alpha_program,
      rule hash_collision_budget_ro_staged_alpha_program)

lemma hash_range_collision_budget_ro_staged_trace_fri_program:
  ...
  by (rule hash_range_collision_budgetI)
    (rule hash_range_budget_ro_staged_trace_fri_program[OF controlled bound],
      rule hash_collision_budget_ro_staged_trace_fri_program[OF controlled bound])

lemma hash_range_collision_budget_guarded_ro_staged_composition_fri_program:
  ...
  by (rule hash_range_collision_budgetI)
    (rule hash_range_budget_guarded_ro_staged_composition_fri_program
        [OF wf controlled],
      rule hash_collision_budget_guarded_ro_staged_composition_fri_program
        [OF wf controlled])

lemma hash_range_collision_budget_ro_checked_staged_query_program_closed:
  assumes controlled: "staged_adversary_controlled budgets A"
  ...
  by (rule hash_range_collision_budgetI)
    (rule hash_range_budget_ro_checked_staged_query_program_closed
        [OF controlled bound trace_len composition_len],
      rule hash_collision_budget_ro_checked_staged_query_program_closed
        [OF controlled bound trace_len composition_len])

lemma hash_range_collision_budget_ro_checked_staged_transcript_program:
  assumes wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
  shows
    "hash_range_collision_budget
...
proof -
  ...
  have trace_root_range:
    "hash_range_collision_budget ?trace_root (trace_root_stage A)"
    using controlled unfolding staged_adversary_controlled_def
    by (blast intro: controlled_ro_program_range_collision)
  have trace_fri_range:
    "hash_range_collision_budget ?trace_fri
    ...
    have exact:
      "hash_range_collision_budget
        (sum_list
          ...
          2 * ceil_log clength)
        (ro_staged_trace_fri_program A 0 (ceil_log clength) [])"
      by (rule hash_range_collision_budget_ro_staged_trace_fri_program
          [OF controlled len])
    then show ?thesis
      using wf unfolding staged_budget_wellformed_def by simp
  qed
  have trace_final_range:
    "\<And>bs. hash_range_collision_budget ?trace_final (trace_final_stage A bs)"
    using controlled unfolding staged_adversary_controlled_def
    by (blast intro: controlled_ro_program_range_collision)
  have alpha_range:
    "hash_range_collision_budget ?alpha (ro_staged_alpha_program (length spec))"
    by (rule hash_range_collision_budget_ro_staged_alpha_program)
  have degree_range:
    "\<And>as. hash_range_collision_budget ?degree (degree_stage A as)"
    using controlled unfolding staged_adversary_controlled_def
    by (blast intro: controlled_ro_program_range_collision)
  have composition_fri_range:
    "\<And>dg. hash_range_collision_budget ?composition_fri
      (assert (ceil_log (to_nat dg + 1) \<le> ceil_log (maxDegree + 1)) \<bind>
        (\<lambda>_. ro_staged_composition_fri_program A dg 0
          (ceil_log (to_nat dg + 1)) []))"
    by (rule hash_range_collision_budget_guarded_ro_staged_composition_fri_program
        [OF wf controlled])
  have composition_final_range:
    "\<And>dg bs. hash_range_collision_budget ?composition_final
  ...
  have query_range:
    "\<And>(trace_roots :: 'f list) (composition_roots :: 'f list).
      ...
      hash_range_collision_budget ?query
  ...
  proof -
    ...
    have exact:
      "hash_range_collision_budget
        ...
        (ro_checked_staged_query_program A trace_roots composition_roots
          0 rounds)"
      by (rule hash_range_collision_budget_ro_checked_staged_query_program_closed
  ...
  proof (rule hash_range_collision_budget_bind_on_outcomes)
    ...
    fix dg F
    show "hash_range_collision_budget ?composition_fri
      (assert (ceil_log (to_nat dg + 1) \<le> ceil_log (maxDegree + 1)) \<bind>
        (\<lambda>_. ro_staged_composition_fri_program A dg 0
          (ceil_log (to_nat dg + 1)) []))"
  ...
  next
    ...
    show "hash_range_collision_budget (?composition_final + (1 + (?query + 0)))
    ...
    proof (cases x)
      ...
      have "hash_range_collision_budget (?composition_final + (1 + (?query + 0)))
        (composition_final_stage A dg composition_bs \<bind>
          (\<lambda>composition_final.
            ro_record_staged_message composition_final \<bind>
              (\<lambda>_. ro_checked_staged_query_program A trace_roots
  ...
  have after_trace_fri:
    "\<And>F. hash_range_collision_budget
      (?trace_fri +
        (?trace_final +
          (1 +
            (?alpha +
  ...
  proof (rule hash_range_collision_budget_bind_on_outcomes)
    fix F
    show "hash_range_collision_budget ?trace_fri
      (ro_staged_trace_fri_program A 0 (ceil_log clength) [])"
  ...
  next
    ...
    proof (cases x)
      ...
      have trace_len: "length trace_roots = ceil_log clength"
        ...
        by simp
      have "hash_range_collision_budget
        (?trace_final +
          (1 +
            (?alpha +
              (?degree +
  ...
  have after_trace_root_record:
    "\<And>fr F. hash_range_collision_budget
      (1 +
        (?trace_fri +
          (?trace_final +
            (1 +
                                  ...
                                  (\<lambda>query_chunks.
                                    return
                                      (F trace_roots trace_bs trace_final as
                                        ...
                                        composition_final query_chunks)))))))))))))"
    by (rule hash_range_collision_budget_bind)
      (rule hash_range_collision_budget_ro_record_staged_message, rule after_trace_fri)
  have whole:
    "hash_range_collision_budget
      (?trace_root +
        (1 +
  ...
  proof (rule hash_range_collision_budget_bind)
    show "hash_range_collision_budget ?trace_root (trace_root_stage A)"
...
lemma hash_collision_budget_ro_checked_staged_transcript_program:
  assumes wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
  shows
  ...
  by (rule hash_range_collision_budget_collision
      [OF hash_range_collision_budget_ro_checked_staged_transcript_program
        [OF wf controlled]])
