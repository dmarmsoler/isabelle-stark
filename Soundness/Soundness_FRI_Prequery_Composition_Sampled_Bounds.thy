(*  Title:      Stark/Soundness_FRI_Prequery_Composition_Sampled_Bounds.thy
    Author:     Diego Marmsoler
    Maintainer: Diego Marmsoler
    License:    BSD-3-Clause
*)

theory Soundness_FRI_Prequery_Composition_Sampled_Bounds
  imports Soundness_FRI_Prequery_Trace_Sampled_Bounds
begin

text \<open>
  Staged bounds for composition FRI sampled-layer events.  This theory mirrors
  the trace-side prequery accounting with the composition degree parameter.
\<close>

context soundness
begin

lemma checked_staged_security_composition_fri_sampled_layer_chain_bound_from_fresh_and_prequery:
...
proof -
  ...
  proof (rule wp_event_mono_on_support)
    ...
    assume support:
      ...
      and hit:
      "staged_security_with_data_state_verifier_event
        composition_fri_bad_with_sampled_layer_chain out"
...
lemma checked_staged_security_composition_fri_verifier_tied_sampled_layer_chain_bound_from_fresh_and_prequery:
  assumes wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
    and finite_B: "\<And>dg. finite (B dg)"
    and cover:
...
proof -
  ...
  also have "... \<le>
      ...
      hash_relation_budget_value
        ...
        (staged_attacker_query_budget budgets + staged_challenge_query_budget)"
    by (rule
        checked_staged_security_composition_fri_sampled_layer_chain_bound_from_fresh_and_prequery
