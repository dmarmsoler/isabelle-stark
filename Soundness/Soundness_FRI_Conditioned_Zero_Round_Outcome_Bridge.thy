theory Soundness_FRI_Conditioned_Zero_Round_Outcome_Bridge
  imports
    Stark.Soundness_FRI_RO_Actual_Query_Zero_Round_Composition_Classification
...
lemma checked_builder_zero_round_conditioned_residual_imp_bounded_relation_transition:
...
proof -
  ...
  from ro_checked_staged_transcript_zero_round_header_query_prefix_chain[
      OF zero wf controlled outcome clean zero_bound]
  obtain header_fr where chains:
  ...
  have counter0: "PQueryCounter (query_states ! 0) = 0"
    using witness_props zero_fields zero_bound by simp
  have lookup0_raw:
      "fmlookup (HashMap attacker_state)
        (QueryIndexChallenge
  ...
  have lookup0:
      "fmlookup (HashMap attacker_state)
        ...
        Some (raws ! 0)"
    using lookup0_raw counter0 by simp
  have final_map:
  ...
  have active:
    ...
    by (rule ro_conditioned_residual_absorbed_query_relation_activeI[
      ...
      (use shape degree_bound header_chain query_chain outcome_props residual
        zero_bound lookup0 in simp_all)
  have domain:
...
lemma checked_builder_zero_round_composition_padded_candidate_bad_imp_conditioned_transitions_or_targets:
      ...
      next
        ...
        have transition:
            "hash_state_relation_transition
              ...
              (HashMap adversary_initial_state) (HashMap attacker_state)"
          by (rule
            checked_builder_zero_round_conditioned_residual_imp_bounded_relation_transition[
              OF zero wf controlled builder_out attacker_clean no_initial
...
lemma checked_builder_zero_round_composition_padding_bad_imp_transition_or_targets:
  fixes final_state :: "'f protocol_channel"
  assumes wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
    and zero: "ceil_log clength = 0"
  ...
  shows
    "hash_map_new_output_hit
     ...
     hash_state_relation_transition
        ...
        (HashMap adversary_initial_state) (HashMap attacker_state)"
proof -
  from ro_absorb_checked_staged_zero_round_actual_query_conceptual_consistency_or_targets[
      OF zero wf controlled builder_out verifier_out final_clean]
  ...
  proof
    ...
    obtain zero_fr where zero_fields:
        "prefix = (zero_fr, [], zero_fr) \<and>
         staged_trace_root data = zero_fr \<and>
         prefix_state \<le> query_start \<and>
         query_start \<le> attacker_state \<and>
         PQueryCounter query_start = 0"
      by blast
    ...
    proof
      show "prefix_state \<le> attacker_state"
        by (rule hash_ext_trans)
          (use zero_fields in blast)+
      show "PQueryCounter query_start = 0"
    ...
    have degree_bound: "to_nat (staged_degree data) \<le> maxDegree"
      using ro_absorb_checked_staged_zero_round_actual_query_accepted_evidence[
        OF zero wf controlled builder_out verifier_out final_clean]
      by blast
    have rounds_pos: "0 < rounds" by (rule rounds_positive)
    from ro_checked_staged_transcript_zero_round_header_query_prefix_chain[
        OF zero wf controlled builder_out attacker_clean rounds_pos]
    obtain header_fr where header_chains:
        "prefix = (header_fr, [], header_fr) \<and>
         staged_trace_root data = header_fr \<and>
    ...
    have chains:
        "prefix =
          (staged_trace_root data, [], staged_trace_root data) \<and>
         ro_absorb_lookup_chain attacker_state (PState adversary_initial_state)
           (verifier_header_messages (staged_trace_root data)
             (staged_trace_fri_roots data) (staged_trace_final data)
             (staged_alphas data) (staged_degree data)
    ...
    next
      ...
      next
        ...
        have domain:
          ...
          by (rule
            ro_checked_staged_transcript_program_with_first_root_map_domain_bound_all_rounds[
              OF wf controlled builder_out attacker_clean])
        have transition:
            "hash_state_relation_transition
              (conditioned_fri_relation_bounded
                (ro_checked_staged_transcript_hash_query_budget_for budgets)
