theory Soundness_FRI_Robust_Balanced_Selected_Query_Phase_Transport
  imports
    Stark.Soundness_FRI_Robust_Query_Phase_Transport
    Stark.Soundness_FRI_Robust_Balanced_Selected_Actual_Query
...
lemma fri_balanced_selected_trace_residual_query_lists_stable_if_no_builder_target:
  assumes ext: "s \<le> t"
    and no_target:
...
proof -
  ...
  have active:
      "fri_balanced_residual_active_layers (clength - 1) C
          ...
          (staged_trace_final data) t =
        fri_balanced_residual_active_layers (clength - 1) C
          (staged_trace_fri_roots data)
          (staged_trace_fri_challenges data)
          (staged_trace_final data) s"
    unfolding fri_balanced_residual_active_layers_def
    by (simp add: layers)
  show ?thesis
    unfolding fri_balanced_selected_trace_residual_query_lists_def
      fri_balanced_selected_residual_query_lists_def Let_def
    by (simp only: active layers)
qed

lemma fri_balanced_selected_composition_residual_query_lists_stable_if_no_builder_target:
  assumes ext: "s \<le> t"
    and no_target:
      "\<not> hash_map_new_output_hit
        (fri_checked_builder_merkle_targets data s) s t"
...
proof -
  ...
  have active:
      "fri_balanced_residual_active_layers
          ...
          (staged_composition_final data) t =
        fri_balanced_residual_active_layers
          (to_nat (staged_degree data)) C
          (staged_composition_fri_roots data)
          (staged_composition_fri_challenges data)
  ...
  show ?thesis
    unfolding fri_balanced_selected_composition_residual_query_lists_def
      fri_balanced_selected_residual_query_lists_def Let_def
    by (simp add: layers active)
qed

lemma fri_balanced_selected_combined_query_head_lists_stable_if_no_builder_target:
  assumes ext: "s \<le> t"
    and no_target:
      "\<not> hash_map_new_output_hit
        (fri_checked_builder_merkle_targets data s) s t"
  ...
  unfolding fri_balanced_selected_combined_query_head_lists_def
    fri_balanced_selected_trace_query_head_lists_def
    fri_balanced_selected_composition_query_head_lists_def
  using
...
lemma checked_builder_balanced_selected_residual_imp_actual_or_query_phase_target:
...
next
  ...
  have stable:
    ...
    by (rule
      fri_balanced_selected_combined_query_head_lists_stable_if_no_builder_target[
        OF _ False])
      (use props in blast)
  have residual_start:
      "map (\<lambda>raw. index (to_nat raw)) raws \<in>
