theory Soundness_FRI_Robust_Trace_Classification
  imports
...
lemma checked_builder_trace_not_robust_close_imp_residual_or_targets:
...
proof -
        ...
        qed      have initial_not_close:
          "\<not> fri_canonical_layer_close (clength - 1)
            ...
            (fri_linear_radius (length (map fst f_fl)) K) 0"
      ...
      have exponent_fit:
          "length (map fst f_fl) + K \<le> N"
        using challenge_len trace_exponent_fit by simp
      have capacity:
          "16 * Suc (length (map fst f_fl)) \<le> 2 ^ K"
        using challenge_len trace_capacity by simp
      have split_full:
          "fri_canonical_layer_close (clength - 1)
              (fri_robust_conditioned_layers (length (map fst f_fl))
                (fri_builder_conceptual_layers (map snd f_fl)
                  attacker_state f_final))
              (fri_linear_radius (length (map fst f_fl)) K) 0 \<or>
            (\<exists>j < length (map fst f_fl).
              map fst f_fl ! j \<in>
                fri_online_robust_bad_challenges
                  (clength - 1) (length (map fst f_fl)) K j
            ...
            (\<exists>i < length (map fst f_fl).
              \<not> fri_canonical_layer_close (clength - 1)
                (fri_robust_conditioned_layers (length (map fst f_fl))
                  (fri_builder_conceptual_layers (map snd f_fl)
                    attacker_state f_final))
                (fri_linear_radius (length (map fst f_fl)) K) i \<and>
              fri_canonical_layer_close (clength - 1)
                (fri_robust_conditioned_layers (length (map fst f_fl))
                  (fri_builder_conceptual_layers (map snd f_fl)
                    attacker_state f_final))
                (fri_linear_radius (length (map fst f_fl)) K) (Suc i) \<and>
              ?query_idxs \<in>
        ...
        by (rule fri_builder_authenticated_chain_robust_online_or_residual[
          OF trace_chain eval_power round_count exponent_fit capacity
            trace_initial_rate attacker_final_ext attacker_clean
            no_builder_trace_target trace_authenticated])
      have split:
          "(\<exists>j < length (map fst f_fl).
              map fst f_fl ! j \<in>
                fri_online_robust_bad_challenges
      ...
      proof
        ...
        have staged_online:
            "\<exists>j < length (staged_trace_fri_roots data).
              staged_trace_fri_challenges data ! j \<in>
                fri_online_robust_bad_challenges
                  (clength - 1)
                  (length (staged_trace_fri_roots data)) K j
        ...
        have transition:
          ...
          by (rule
            checked_builder_trace_robust_online_bad_imp_bounded_relation_transition[
        ...
        have quantitative:
            "?query_idxs \<in>
              fri_robust_quantitative_residual_query_lists
                (clength - 1) K (map snd f_fl) (map fst f_fl)
