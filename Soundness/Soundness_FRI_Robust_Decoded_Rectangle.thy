theory Soundness_FRI_Robust_Decoded_Rectangle
  imports
...
definition robust_decoded_semantic_query_index_bound :: "nat \<Rightarrow> nat"
where
  "robust_decoded_semantic_query_index_bound K =
    trace_composition_padding_query_index_bound +
    fri_linear_radius (ceil_log clength) K 0 +
    fri_linear_radius (ceil_log (Suc maxDegree)) K 0"

definition ro_robust_decoded_semantic_query_indices
where
  "ro_robust_decoded_semantic_query_indices
      K prefix prefix_state data query_start =
    (let d = to_nat (staged_degree data);
         ...
         trace_radius = fri_linear_radius (ceil_log clength) K 0;
         composition_radius =
           fri_linear_radius (ceil_log (Suc d)) K 0
     in if
       d \<le> maxDegree \<and>
...
definition ro_robust_decoded_semantic_query_lists
where
  "ro_robust_decoded_semantic_query_lists
      K prefix prefix_state data query_start =
...
lemma ro_robust_decoded_semantic_query_lists_rectangle_cover:
  "ro_robust_decoded_semantic_query_lists
      K prefix prefix_state data query_start
    \<subseteq> fri_conditioned_query_lists
      (ro_robust_decoded_semantic_query_indices
        K prefix prefix_state data query_start)"
  unfolding ro_robust_decoded_semantic_query_lists_def by simp

lemma ro_robust_decoded_semantic_query_indices_subset:
  "ro_robust_decoded_semantic_query_indices
      K prefix prefix_state data query_start \<subseteq> query_sample_space"
  unfolding ro_robust_decoded_semantic_query_indices_def Let_def
  using ro_actual_query_trace_composition_accepted_indices_subset[
    of prefix prefix_state data query_start]
  by auto

lemma trace_composition_query_index_bound_le_padding:
...
lemma fri_linear_radius_initial_mono:
  assumes "m \<le> M"
  shows "fri_linear_radius m K 0 \<le> fri_linear_radius M K 0"
  using assms
  unfolding fri_linear_radius_def
  by (simp add: mult_right_mono)


lemma card_ro_robust_decoded_semantic_query_indices:
  "card (ro_robust_decoded_semantic_query_indices
      K prefix prefix_state data query_start)
    \<le> robust_decoded_semantic_query_index_bound K"
proof -
  let ?d = "to_nat (staged_degree data)"
  let ?fri_table =
    "first_trace_fri_root_prefix_first_table prefix prefix_state"
  ...
  let ?trace_radius = "fri_linear_radius (ceil_log clength) K 0"
  ...
  next
    ...
    have composition_radius_bound:
        "?composition_radius \<le>
          fri_linear_radius (ceil_log (Suc maxDegree)) K 0"
    proof (rule fri_linear_radius_initial_mono)
    ...
    proof -
      ...
      proof
        ...
        show ?thesis
          ...
          unfolding robust_decoded_semantic_query_index_bound_def
          by linarith
      next
        assume rest:
            "(composition_table_low_degree maxDegree ?decoded_composition \<and>
...
definition ro_checked_staged_robust_decoded_semantic_rectangle_error_adaptive
...
lemma
  ...
  shows
    ...
    ro_checked_staged_robust_decoded_semantic_rectangle_error_adaptive
      K budgets"
  unfolding
    ro_checked_staged_robust_decoded_semantic_rectangle_error_adaptive_def
proof (rule
    wp_ro_query_head_dependent_actual_query_index_list_hit_adaptive_rectangle_bound[
      OF wf controlled,
      where I="ro_robust_decoded_semantic_query_indices K"])
  ...
  show
    "ro_robust_decoded_semantic_query_lists
      ...
      \<subseteq> fri_conditioned_query_lists
        (ro_robust_decoded_semantic_query_indices
          K prefix prefix_state data query_start)"
    by (rule ro_robust_decoded_semantic_query_lists_rectangle_cover)
next
  fix prefix prefix_state data query_start t
  assume
    "Some (((prefix, prefix_state), data, query_start), t) \<in>
  ...
  show
    "ro_robust_decoded_semantic_query_indices
        K prefix prefix_state data query_start \<subseteq> query_sample_space"
    by (rule ro_robust_decoded_semantic_query_indices_subset)
next
  fix prefix prefix_state data query_start t
  assume
    "Some (((prefix, prefix_state), data, query_start), t) \<in>
  ...
  show
    "card
      (ro_robust_decoded_semantic_query_indices
        K prefix prefix_state data query_start)
      \<le> robust_decoded_semantic_query_index_bound K"
    by (rule card_ro_robust_decoded_semantic_query_indices)
qed
