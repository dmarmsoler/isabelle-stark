theory Soundness_FRI_Robust_Balanced_Parameter_Bound
  imports
    Stark.Soundness_FRI_Robust_Exact_Rate_Parameter_Bound
    Stark.Soundness_FRI_Robust_Balanced_Security_Classification
...
definition ro_checked_staged_balanced_trace_challenge_error
...
definition ro_checked_staged_balanced_composition_challenge_error
  :: "nat \<Rightarrow> staged_budgets \<Rightarrow> prob"
where
...
definition ro_absorb_checked_staged_balanced_nonempty_parameter_error
  :: "nat \<Rightarrow> staged_budgets \<Rightarrow> prob"
where
  "ro_absorb_checked_staged_balanced_nonempty_parameter_error C budgets =
...
lemma
  ...
  shows
    ...
    \<le> ro_checked_staged_balanced_decoded_semantic_rectangle_error_adaptive
      C budgets"
  by (rule order_trans[
      OF
...
lemma
  wp_ro_absorb_checked_staged_security_combined_balanced_residual_actual_rectangle_bound_adaptive:
  assumes wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
  ...
  shows
    "wp_event
      ...
      adversary_initial_state
    \<le> ro_checked_staged_balanced_combined_actual_residual_rectangle_error_adaptive
      C budgets"
  by (rule order_trans[
      OF
        wp_ro_absorb_checked_staged_security_query_head_dependent_actual_query_index_list_hit_le
...
lemma
  ...
  shows
    "wp_event
      ...
      adversary_initial_state
    \<le> ro_checked_staged_balanced_trace_challenge_error C budgets"
  unfolding ro_checked_staged_balanced_trace_challenge_error_def Let_def
  by (rule order_trans[
      OF
...
lemma
  ...
  shows
    "wp_event
      ...
      adversary_initial_state
    \<le> ro_checked_staged_balanced_composition_challenge_error C budgets"
  unfolding ro_checked_staged_balanced_composition_challenge_error_def Let_def
  by (rule order_trans[
      OF
        wp_ro_absorb_checked_staged_security_builder_relation_transition_le
...
lemma
  wp_ro_absorb_checked_staged_security_acceptance_balanced_nonempty_parameter_bound_exact_two:
  ...
  shows
    "wp_event
      ...
      \<le> ro_absorb_checked_staged_balanced_nonempty_parameter_error C budgets"
proof -
  let ?M =
    "ro_absorb_checked_staged_security_experiment_with_first_root_and_query_witnesses
      A"
  ...
  proof -
    ...
    show ?thesis
      using summed
      unfolding ro_absorb_checked_staged_balanced_nonempty_parameter_error_def
      by (simp add: add.assoc)
  qed
  show ?thesis
...
lemma ro_absorb_stark_soundness_balanced_nonempty_exact_two:
  assumes false_statement: "\<not> exists_valid_trace"
    and wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
    and nonempty: "0 < ceil_log clength"
...
definition
  ro_absorb_checked_staged_balanced_tightened_nonempty_parameter_error
    :: "nat \<Rightarrow> staged_budgets \<Rightarrow> prob"
where
  "ro_absorb_checked_staged_balanced_tightened_nonempty_parameter_error C budgets =
    min
...
lemma
  ro_absorb_checked_staged_balanced_tightened_nonempty_parameter_error_le_previous:
  "ro_absorb_checked_staged_balanced_tightened_nonempty_parameter_error C budgets
    \<le>
    ro_absorb_checked_staged_conditioned_explicit_complete_list_adaptive_rectangle_tightened_nonempty_parameter_error
      budgets"
  unfolding
    ro_absorb_checked_staged_balanced_tightened_nonempty_parameter_error_def
  by simp
...
lemma ro_absorb_stark_soundness_balanced_tightened_nonempty:
  assumes false_statement: "\<not> exists_valid_trace"
    and wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
    and nonempty: "0 < ceil_log clength"
