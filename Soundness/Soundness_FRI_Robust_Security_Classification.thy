theory Soundness_FRI_Robust_Security_Classification
  imports
    Soundness_FRI_Robust_Composition_Classification
    Stark.Soundness_FRI_Robust_Decoded_Alpha_Pivot
...
lemma ro_robust_builder_side_events_lift:
  assumes out_eq:
      "out = Some
    ...
    and attacker_clean: "\<not> hash_map_output_collision attacker_state"
    and side:
  ...
  shows
    "ro_absorb_checked_staged_security_clean_builder_merkle_target_hit out \<or>
     ro_absorb_checked_staged_security_builder_relation_transition R out \<or>
...
proof
  ...
  have event:
      "ro_absorb_checked_staged_security_clean_builder_merkle_target_hit out"
...
next
  ...
  proof
    ...
    have event:
        "ro_absorb_checked_staged_security_builder_relation_transition R out"
      using transition
      unfolding out_eq
    ...
    proof
      ...
      have event:
        ...
        unfolding out_eq
          ...
          ro_query_head_data_def fri_checked_builder_merkle_targets_def
        by simp
      then show ?thesis by blast
    next
      assume actual:
...
lemma
  ro_absorb_checked_staged_security_clean_robust_classification:
  ...
  shows
    "ro_absorb_checked_staged_security_query_head_dependent_actual_query_index_list_hit
        (ro_robust_decoded_semantic_query_lists K) out \<or>
     ...
     ro_absorb_checked_staged_security_builder_relation_transition
       (conditioned_fri_relation_bounded
         ...
         (robust_conditioned_trace_fri_bad_challenge_relation K)) out \<or>
     ro_absorb_checked_staged_security_builder_relation_transition
       (conditioned_fri_relation_bounded
         (ro_checked_staged_transcript_hash_query_budget_for budgets)
         (robust_conditioned_composition_fri_bad_challenge_relation K)) out \<or>
     ro_absorb_checked_staged_security_query_head_dependent_actual_query_index_list_hit
       (fri_robust_combined_query_head_lists K) out \<or>
     ro_absorb_checked_staged_security_query_phase_builder_merkle_target_hit
       out \<or>
     ro_absorb_checked_staged_security_robust_decoded_all_queries_consistent
       out"
          ...
          next
            ...
            have semantic_member:
              ...
              using degree_bound trace_close composition_close False
                accepted_queries
              unfolding ro_robust_decoded_semantic_query_lists_def
                ro_robust_decoded_semantic_query_indices_def Let_def
            ...
            have event:
                "ro_absorb_checked_staged_security_query_head_dependent_actual_query_index_list_hit
                  (ro_robust_decoded_semantic_query_lists K) out"
            ...
            "hash_map_new_output_hit
             ...
             ro_query_head_dependent_actual_query_index_list_hit
                ...
                (Some ((((prefix, prefix_state), data, query_start, raws,
                  query_states), attacker_state)))"
          by (rule
            checked_builder_composition_not_robust_close_imp_residual_or_targets[
              OF wf controlled nonempty eval_power composition_exponent_fit
                composition_capacity composition_initial_rate builder_out
                verifier_out final_clean no_initial composition_far])
        then show ?thesis
        proof
          assume composition_target:
              "hash_map_new_output_hit
          ...
          have lifted:
              "ro_absorb_checked_staged_security_clean_builder_merkle_target_hit
               ...
               ro_absorb_checked_staged_security_query_phase_builder_merkle_target_hit
                  out \<or>
               ro_absorb_checked_staged_security_query_head_dependent_actual_query_index_list_hit
                  (fri_robust_combined_query_head_lists K) out"
          ...
          "hash_map_new_output_hit
           ...
           ro_query_head_dependent_actual_query_index_list_hit
              ...
              (Some ((((prefix, prefix_state), data, query_start, raws,
                query_states), attacker_state)))"
        by (rule
          checked_builder_trace_not_robust_close_imp_residual_or_targets[
            OF wf controlled nonempty eval_power trace_exponent_fit
              trace_capacity trace_initial_rate builder_out verifier_out
              final_clean no_initial trace_far])
      then show ?thesis
      proof
        assume trace_target:
            "hash_map_new_output_hit
...
definition ro_absorb_checked_staged_robust_obstruction_union
where
  "ro_absorb_checked_staged_robust_obstruction_union K budgets out \<longleftrightarrow>
...
lemma
  wp_ro_absorb_checked_staged_security_acceptance_le_robust_obstruction_union:
  assumes wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
    and nonempty: "0 < ceil_log clength"
    and eval_power: "clength * scale = 2 ^ N"
...
proof (rule wp_event_mono_on_support)
  ...
  assume support:
    ...
    and accepted_out: "accepted out"
  show
      "ro_absorb_checked_staged_robust_obstruction_union K budgets out"
  ...
  next
    ...
    from
      ro_absorb_checked_staged_security_clean_robust_classification[
        OF wf controlled nonempty eval_power trace_exponent_fit
          composition_exponent_fit trace_capacity composition_capacity
          trace_initial_rate composition_initial_rate accepted_out support
          False]
    show ?thesis
      unfolding ro_absorb_checked_staged_robust_obstruction_union_def
      by blast
