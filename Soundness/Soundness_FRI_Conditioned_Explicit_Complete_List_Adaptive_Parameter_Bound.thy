theory Soundness_FRI_Conditioned_Explicit_Complete_List_Adaptive_Parameter_Bound
  imports
...
definition
  ro_absorb_checked_staged_conditioned_explicit_complete_list_adaptive_nonempty_parameter_error
...
lemma
  wp_ro_absorb_checked_staged_security_acceptance_conditioned_explicit_complete_list_adaptive_nonempty_parameter_bound:
  ...
  shows
    "wp_event
      ...
      \<le> ro_absorb_checked_staged_conditioned_explicit_complete_list_adaptive_nonempty_parameter_error
          budgets"
proof -
  let ?M =
    "ro_absorb_checked_staged_security_experiment_with_first_root_and_query_witnesses
  ...
  have query:
      "wp_event ?M ?Query adversary_initial_state \<le>
        ro_checked_staged_trace_composition_complete_list_good_actual_query_error_adaptive budgets"
    by (rule
  ...
  have actual_residual:
      "wp_event ?M ?ActualResidual adversary_initial_state \<le>
        ro_checked_staged_conditioned_complete_list_actual_residual_error_adaptive budgets"
    by (rule
  ...
  have padding:
      "wp_event ?M ?Padding adversary_initial_state \<le>
        ro_checked_staged_trace_composition_complete_list_padding_error_adaptive budgets"
    by (rule
  ...
  proof -
    ...
    show ?thesis
      using summed
      unfolding
        ro_absorb_checked_staged_conditioned_explicit_complete_list_adaptive_nonempty_parameter_error_def
      by (simp add: add.assoc)
  qed
  show ?thesis
...
lemma
  ro_absorb_checked_staged_conditioned_explicit_complete_list_adaptive_tightened_nonempty_parameter_error_le_old:
  "ro_absorb_checked_staged_conditioned_explicit_complete_list_adaptive_tightened_nonempty_parameter_error
      budgets
    \<le> ro_absorb_checked_staged_conditioned_explicit_nonempty_parameter_error
        budgets"
  unfolding
    ro_absorb_checked_staged_conditioned_explicit_complete_list_adaptive_tightened_nonempty_parameter_error_def
  by simp
...
proof -
  ...
  show ?thesis
    unfolding
      ro_absorb_checked_staged_conditioned_explicit_complete_list_adaptive_tightened_nonempty_parameter_error_def
    by (rule min.boundedI[OF old adaptive])
qed

lemma ro_absorb_stark_soundness_conditioned_explicit_complete_list_adaptive_nonempty:
  assumes false_statement: "\<not> exists_valid_trace"
    and wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
    and nonempty: "0 < ceil_log clength"
...
lemma ro_absorb_stark_soundness_conditioned_explicit_complete_list_adaptive_tightened_nonempty:
  assumes false_statement: "\<not> exists_valid_trace"
    and wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
    and nonempty: "0 < ceil_log clength"
