theory Soundness_FRI_RO_Actual_Query_Trace_Composition_Alpha_Pivot_Adaptive_Query_Budget
  imports
...
definition alpha_pivot_absorbed_query_relation_bounded
  :: "nat \<Rightarrow>
      (('f protocol_hash_input, 'f) fmap \<Rightarrow>
        'f protocol_hash_input \<Rightarrow> 'f \<Rightarrow> bool)"
where
  "alpha_pivot_absorbed_query_relation_bounded L M x y \<longleftrightarrow>
    card (fmdom' M) \<le> L \<and>
    alpha_pivot_absorbed_query_relation M x y"


lemma alpha_pivot_bounded_direct_activation_subset:
  assumes fresh: "fmlookup M x = None"
  shows
    "{y.
      hash_state_relation_direct_activation
        (alpha_pivot_absorbed_query_relation_bounded L) M x y}
      \<subseteq>
    {y.
      hash_state_relation_direct_activation
        alpha_pivot_absorbed_query_relation M x y}"
proof
  ...
  have active_new:
      "hash_state_relation_active
        (alpha_pivot_absorbed_query_relation_bounded L)
        (fmupd x y M) x y"
...
lemma alpha_pivot_bounded_direct_fiber_card_bound:
  assumes fresh: "fmlookup M x = None"
  shows
    "card {y.
      hash_state_relation_direct_activation
...
proof -
  ...
  have subset: "?A \<subseteq> ?B"
    by (rule alpha_pivot_bounded_direct_activation_subset[OF fresh])
  have finite_B: "finite ?B"
  proof -
    have "card ?B \<le>
        1"
      by (rule
        alpha_pivot_absorbed_query_relation_direct_fiber_card_bound)
...
lemma alpha_pivot_bounded_drift_activation_subset:
  assumes fresh: "fmlookup M x = None"
  shows
    "{y.
...
proof
  ...
  then obtain k z where
    ...
    and inactive_old:
      "\<not> hash_state_relation_active
        (alpha_pivot_absorbed_query_relation_bounded L) M k z"
...
lemma alpha_pivot_bounded_drift_fiber_card_bound:
  assumes fresh: "fmlookup M x = None"
  ...
  let ?B =
    "{y.
      hash_state_relation_drift_activation
        alpha_pivot_absorbed_query_relation M x y}"
  have subset: "?A \<subseteq> ?B"
    by (rule alpha_pivot_bounded_drift_activation_subset[OF fresh])
  have finite_B: "finite ?B"
  proof -
    have "card ?B \<le> 6 * card (fmdom' M) + 2"
      by (rule
        alpha_pivot_absorbed_query_relation_drift_fiber_card_bound[
          OF fresh])
    then show ?thesis by simp
  qed
  have "card ?A \<le> card ?B"
...
lemma alpha_pivot_bounded_transition_fiber_card_bound:
  assumes fresh: "fmlookup M x = None"
  shows
    "card {y.
  ...
  have direct:
      "card {y.
        hash_state_relation_direct_activation
          (alpha_pivot_absorbed_query_relation_bounded L) M x y}
        \<le> 1"
    by (rule alpha_pivot_bounded_direct_fiber_card_bound[OF fresh])
  have drift:
      "card {y.
        hash_state_relation_drift_activation
          (alpha_pivot_absorbed_query_relation_bounded L) M x y}
        \<le> 6 * L + 2"
    by (rule
      alpha_pivot_bounded_drift_fiber_card_bound[OF fresh True])
  show ?thesis
...
next
  ...
  proof
    ...
    have domain_new: "card (fmdom' (fmupd x y M)) \<le> L"
      using active_new
      unfolding hash_state_relation_active_def
        alpha_pivot_absorbed_query_relation_bounded_def
      by blast
    have domain_old: "card (fmdom' M) \<le> L"
      using card_fmdom_fmupd_mono[of M x y] domain_new
      by linarith
...
lemma wp_ro_checked_staged_transcript_program_with_first_root_and_query_witnesses_alpha_pivot_bounded_relation:
      ...
      "\<And>M x. fmlookup M x = None \<Longrightarrow>
        card {y.
          hash_state_relation_transition
            ...
            M (fmupd x y M)}
          \<le> ?B"
    by (rule alpha_pivot_bounded_transition_fiber_card_bound)
  have bound:
      "wp_event
          (ro_checked_staged_transcript_program_with_first_root_and_query_witnesses
            A)
...
lemma wp_ro_checked_staged_transcript_program_with_first_root_and_query_witnesses_alpha_pivot_bounded_relation_all_rounds:
  assumes wf: "staged_budget_wellformed budgets"
    and controlled: "staged_adversary_controlled budgets A"
  defines
    "q \<equiv> ro_checked_staged_transcript_hash_query_budget_for budgets"
