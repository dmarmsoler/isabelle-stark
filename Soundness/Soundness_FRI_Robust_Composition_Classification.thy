theory Soundness_FRI_Robust_Composition_Classification
  imports Soundness_FRI_Robust_Trace_Classification
begin

context soundness
begin

lemma checked_builder_composition_not_robust_close_imp_residual_or_targets:
  ...
  assumes wf: "staged_budget_wellformed budgets"
    ...
    and candidate_far:
      "\<not> fri_rs_distance_to_code
        ...
        \<le> fri_linear_radius
          (ceil_log (Suc (to_nat (staged_degree data)))) K 0"
  shows
    ...
    next
      ...
      have initial_not_close:
          "\<not> fri_canonical_layer_close
            ...
            (fri_linear_radius (length (map fst fl)) K) 0"
      ...
      have exponent_fit:
          "length (map fst fl) + K \<le> N"
        using round_count log_mono composition_exponent_fit by linarith
      have capacity:
          "16 * Suc (length (map fst fl)) \<le> 2 ^ K"
        proof -
          have suc_le:
              "Suc (length (map fst fl)) \<le>
                Suc (ceil_log (Suc maxDegree))"
            using round_count log_mono by simp
          have
              "16 * Suc (length (map fst fl)) \<le>
                16 * Suc (ceil_log (Suc maxDegree))"
            using suc_le by simp
          also have "... \<le> 2 ^ K"
            by (rule composition_capacity)
          finally show ?thesis .
        qed
      have padded_mono:
          "fri_padded_degree_bound (to_nat (staged_degree data)) \<le>
            fri_padded_degree_bound maxDegree"
        by (rule fri_padded_degree_bound_mono[OF degree_bound])
      have initial_rate:
          "4 * fri_padded_degree_bound (to_nat (staged_degree data)) \<le>
            clength * scale"
        using padded_mono composition_initial_rate by linarith
      have split_full:
          "fri_canonical_layer_close (to_nat (staged_degree data))
              (fri_robust_conditioned_layers (length (map fst fl))
                (fri_builder_conceptual_layers (map snd fl)
                  attacker_state final))
              (fri_linear_radius (length (map fst fl)) K) 0 \<or>
            (\<exists>j < length (map fst fl).
              map fst fl ! j \<in>
                fri_online_robust_bad_challenges
                  (to_nat (staged_degree data)) (length (map fst fl)) K j
            ...
            (\<exists>i < length (map fst fl).
              \<not> fri_canonical_layer_close
                ...
                (fri_robust_conditioned_layers (length (map fst fl))
                  (fri_builder_conceptual_layers (map snd fl)
                    attacker_state final))
                (fri_linear_radius (length (map fst fl)) K) i \<and>
              fri_canonical_layer_close
                (to_nat (staged_degree data))
                (fri_robust_conditioned_layers (length (map fst fl))
                  (fri_builder_conceptual_layers (map snd fl)
                    attacker_state final))
                (fri_linear_radius (length (map fst fl)) K) (Suc i) \<and>
              ?query_idxs \<in>
        ...
        by (rule fri_builder_authenticated_chain_robust_online_or_residual[
          OF composition_chain eval_power round_count exponent_fit capacity
            initial_rate attacker_final_ext attacker_clean no_builder_target
            composition_authenticated])
      have split:
          "(\<exists>j < length (map fst fl).
              map fst fl ! j \<in>
                fri_online_robust_bad_challenges
      ...
      proof
        ...
        have transition:
          ...
          by (rule
            checked_builder_composition_robust_online_bad_imp_bounded_relation_transition[
      ...
      next
        ...
        have quantitative:
          ...
          unfolding fri_robust_quantitative_residual_query_lists_def
            fri_robust_residual_active_layers_def
          using query_space residual by blast
        have composition_head:
            "?query_idxs \<in>
              fri_robust_composition_query_head_lists K
                prefix prefix_state data attacker_state"
          using degree_bound quantitative composition_roots_eq
        ...
        have combined_head:
          ...
          unfolding fri_robust_combined_query_head_lists_def by blast
        have actual_or_target:
          ...
          by (rule
            checked_builder_robust_residual_imp_actual_or_query_phase_target[
