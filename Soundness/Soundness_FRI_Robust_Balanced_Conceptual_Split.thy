theory Soundness_FRI_Robust_Balanced_Conceptual_Split
  imports
    Stark.Soundness_FRI_Robust_Exact_Rate_Conceptual_Split
    Stark.Soundness_FRI_Robust_Balanced_Schedule
begin

context soundness
begin

definition fri_balanced_conditioned_bad_challenges ::
...
where
  "fri_balanced_conditioned_bad_challenges d m C committed i prefix =
    (let current = fri_conditioned_layer_table i (committed i prefix);
         good_radius = fri_balanced_good_radius m C i;
...
lemma fri_balanced_conditioned_good_eq_canonical:
  assumes i_bound: "i < m"
    and committed_path:
  ...
  shows
    "fri_balanced_conditioned_bad_challenges d m C committed i
        (take i challenges) =
  ...
  unfolding fri_balanced_conditioned_bad_challenges_def
    fri_canonical_layer_good_challenges_def
    fri_robust_conditioned_layers_nth[OF less_imp_le[OF i_bound]]
    Let_def
  by simp

lemma fri_balanced_conditioned_bad_challenges_card_exact_two:
  fixes d N m C i :: nat
...
proof -
  ...
  show ?thesis
    using split'
    unfolding fri_balanced_conditioned_bad_challenges_def Let_def
    by (auto split: if_splits)
qed


lemma conceptual_sampled_chain_robust_decode_or_reject_balanced_exact_two:
  fixes d N m C :: nat
  ...
  shows
    "fri_canonical_layer_close d
      ...
      challenges \<in> generic_fri_bad_challenge_lists m
        (fri_balanced_conditioned_bad_challenges d m C committed) \<or>
      (\<exists>i<m.
        ...
        (\<exists>i<m.
          ...
          ((challenges ! i \<in>
              fri_canonical_layer_good_challenges d ?conditioned
                (fri_balanced_good_radius m C) i \<and>
           ...
           card (fri_canonical_transition_agreement_indices
              ...
              length (fri_canonical_domain_at (Suc i)) -
                fri_balanced_margin C i))"
    by (rule fri_canonical_multiround_balanced_decode_or_reject_exact_two[
          OF eval_power rounds_eq rounds_fit exact
  ...
  next
    ...
    proof
      ...
      have radius_exact:
          "2 * fri_balanced_good_radius m C i =
            fri_balanced_radius m C i"
        by (rule fri_balanced_current_radius_exact[OF i_bound])
      have current_nth:
          "?conditioned ! i =
            fri_conditioned_layer_table i (layers ! i)"
      ...
      have family_eq:
          "fri_balanced_conditioned_bad_challenges d m C committed i
            ...
            fri_canonical_layer_good_challenges d ?conditioned
              (fri_balanced_good_radius m C) i"
        by (rule fri_balanced_conditioned_good_eq_canonical[
              where d=d and m=m and C=C and committed=committed
      ...
      have bad_list:
          "challenges \<in> generic_fri_bad_challenge_lists m
            (fri_balanced_conditioned_bad_challenges d m C committed)"
...
lemma authenticated_recorded_chain_robust_decode_or_reject_balanced_exact_two:
  fixes d N C :: nat
  assumes chain:
  ...
  shows
    "fri_canonical_layer_close d
      ...
      challenges \<in> generic_fri_bad_challenge_lists
        (length challenges)
        (fri_balanced_conditioned_bad_challenges
          d (length challenges) C committed) \<or>
...
proof -
  ...
  qed
  show ?thesis
    by (rule conceptual_sampled_chain_robust_decode_or_reject_balanced_exact_two[
          where N=N and d=d and m="length challenges" and C=C
            and committed=committed,
          OF challenge_space refl round_count eval_power rounds_fit
...
definition fri_online_balanced_bad_challenges ::
  "nat \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow>
    'f protocol_channel \<Rightarrow> 'f \<Rightarrow> 'f set"
where
  "fri_online_balanced_bad_challenges d m C i state fri_root =
    fri_balanced_conditioned_bad_challenges d m C
      (\<lambda>_ _. conceptual_table state fri_root
        (length (fri_canonical_domain_at i))) i []"

lemma fri_builder_balanced_bad_challenge_cover_imp_online_bad:
  assumes cover:
      "challenges \<in>
        generic_fri_bad_challenge_lists (length challenges)
          (fri_balanced_conditioned_bad_challenges d
            (length challenges) C
...
lemma card_fri_online_balanced_bad_challenges_exact_two:
  fixes d N m C i :: nat
  assumes eval_power: "clength * scale = 2 ^ N"
    and rounds_eq: "m = ceil_log (Suc d)"
    and rounds_fit: "Suc m \<le> N"
  ...
  shows
    "card (fri_online_balanced_bad_challenges
      d m C i state fri_root) \<le>
        2 * fri_balanced_good_radius m C i + 3"
  unfolding fri_online_balanced_bad_challenges_def
  by (rule fri_balanced_conditioned_bad_challenges_card_exact_two[
        OF eval_power rounds_eq rounds_fit exact i_bound])



lemma fri_builder_authenticated_chain_robust_online_or_residual_balanced_exact_two:
  fixes d N C :: nat
  assumes chain:
      "generic_fri_recorded_value_chain_evidence roots challenges final_value
        query_idxs round_layers"
  ...
  have split:
      "fri_canonical_layer_close d
        ...
        challenges \<in> generic_fri_bad_challenge_lists
          (length challenges)
          (fri_balanced_conditioned_bad_challenges d
            (length challenges) C (\<lambda>j _. ?layers ! j)) \<or>
        (\<exists>i<length challenges.
          \<not> fri_canonical_layer_close d
            (fri_robust_conditioned_layers (length challenges) ?layers)
            (fri_balanced_radius (length challenges) C) i \<and>
          ...
          card (fri_robust_conditioned_agreement_indices
            ...
            length (fri_canonical_domain_at (Suc i)) -
              fri_balanced_margin C i)"
    by (rule authenticated_recorded_chain_robust_decode_or_reject_balanced_exact_two[
          where prefix_state="\<lambda>_. builder_state" and layers="?layers"
            and committed="\<lambda>j _. ?layers ! j",
          OF chain eval_power round_count rounds_fit exact])
    ...
    proof
      assume cover:
          "challenges \<in> generic_fri_bad_challenge_lists
            (length challenges)
            (fri_balanced_conditioned_bad_challenges d
              (length challenges) C (\<lambda>j _. ?layers ! j))"
      have online:
          "\<exists>j < length challenges.
            challenges ! j \<in>
              fri_online_balanced_bad_challenges d (length challenges) C j
                builder_state (roots ! j)"
        by (rule fri_builder_balanced_bad_challenge_cover_imp_online_bad[
              OF cover lengths])
      then show ?thesis by simp
    next
      assume residual:
