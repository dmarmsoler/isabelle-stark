theory Soundness_FRI_Robust_Exact_Rate_Composition_Classification
  imports
...
lemma checked_builder_composition_not_robust_close_imp_residual_or_targets_exact_two:
    ...
    next
      ...
      have exact:
          "fri_linear_exact_list_cap
            (to_nat (staged_degree data))
            (length (map fst fl)) K 2"
      proof -
        have at_log:
            "fri_linear_exact_list_cap
              (to_nat (staged_degree data))
              (ceil_log (Suc (to_nat (staged_degree data)))) K 2"
      ...
      have split_full:
        ...
        by (rule
          fri_builder_authenticated_chain_robust_online_or_residual_exact_two[
            OF composition_chain eval_power round_count exponent_fit K_pos exact
